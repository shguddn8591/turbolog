...
pub enum Command {
    ...
    History {
        ...
        /// Group by template, sorted by frequency (most common first)
    ...
    /// Summarize recurring anomaly patterns over a time window (reads local history)
    Diagnose {
        /// Look back this far: 7d, 24h, 1h, 30m (default: 24h)
        #[arg(long, default_value = "24h")]
        since: String,
        /// Output format: "text" (default) or "json"
        #[arg(long, default_value = "text")]
        format: String,
        /// Summarize patterns with a local LLM (auto-detects Ollama or LM Studio)
        #[arg(long)]
        explain: bool,
        /// Local LLM base URL. Also: TURBOLOG_LLM_URL
        #[arg(long)]
        llm_url: Option<String>,
        /// LLM model name. Also: TURBOLOG_LLM_MODEL
        #[arg(long)]
        llm_model: Option<String>,
        /// Maximum number of patterns to include (default: 10)
